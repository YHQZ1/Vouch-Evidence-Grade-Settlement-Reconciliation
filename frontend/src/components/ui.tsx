    ...
    <dl className="grid gap-2 sm:grid-cols-2">
      {values.map((value) => {
        try {
          ...
          return (
            <div key={value.name} className="rounded border border-coral/40 bg-coral/5 p-2" role="alert">
              ...
              <dd className="mt-1 break-all font-mono text-sm font-bold text-coral">
                ...
                <span className="sr-only"> {error instanceof Error ? error.message : ""}</span>
              </dd>
            </div>
          );
        }
...
export function ReadinessBanner({ readiness, counts }: { readiness: Readiness; counts?: string }) {
  const Icon = readiness === "BLOCKED" ? CircleAlert : readiness === "READY" ? ShieldCheck : AlertTriangle;
  return (
    <div className={`flex items-start gap-3 border p-4 ${READINESS_CLASSES[readiness]}`} role="status">
      <Icon size={26} aria-hidden="true" />
      <div>
        <p className="mb-1 text-[11px] font-bold uppercase tracking-[0.16em]">Close readiness</p>
...
export function StateBadge({ state }: { state: ResolutionState }) {
  return (
    <span className={`inline-flex items-center gap-1.5 rounded-full border px-2 py-1 text-xs font-bold ${STATE_CLASSES[state]}`}>
...
export function EvidenceBadge({ status }: { status: "verified" | "proposed" | "rejected" }) {
  return (
    <span className={`inline-flex items-center gap-1 rounded-full border px-2 py-1 text-xs font-bold ${EVIDENCE_CLASSES[status]}`}>
      {status === "verified" ? <ShieldCheck size={13} aria-hidden="true" /> : <Info size={13} aria-hidden="true" />}
      {status}
    </span>
  );
}
  ...
  return (
    <div className="flex flex-wrap gap-1.5">
      {codes.length ? codes.map((code) => (
        <span className="rounded border border-line bg-paper px-2 py-1 text-xs text-muted" key={code} title={code}>
...
export function Loading({ label = "Loading evidence" }: { label?: string }) {
  return <div className="flex items-center gap-2 p-8 text-sm text-muted" role="status"><LoaderCircle className="animate-spin" size={18} aria-hidden="true" />{label}</div>;
}

export function EmptyState({ title, children }: { title: string; children: React.ReactNode }) {
  return <div className="flex items-start gap-3 border border-line bg-panel p-6"><Info size={22} className="mt-0.5 shrink-0 text-teal" aria-hidden="true" /><div><h3 className="font-serif text-xl">{title}</h3><p className="mt-1 text-sm leading-6 text-muted">{children}</p></div></div>;
}

export function ErrorState({ error, onRetry }: { error: Error; onRetry?: () => void }) {
  return <div className="flex items-start gap-3 border border-coral/40 bg-coral/5 p-6" role="alert"><AlertTriangle size={22} className="mt-0.5 shrink-0 text-coral" aria-hidden="true" /><div><h3 className="font-serif text-xl text-coral">Evidence unavailable</h3><p className="mt-1 text-sm leading-6 text-ink">{error.message}</p>{onRetry && <Button className="mt-4" variant="secondary" onClick={onRetry}>Try again</Button>}</div></div>;
}
