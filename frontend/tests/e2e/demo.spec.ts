...
test('completes the frozen evidence-first flow with provenance, exports, and keyboard review', async ({
  page,
}) => {
  ...
  await expect(
    page.getByRole('heading', { name: 'Attach immutable source records' }),
  ).toBeVisible();
  ...
  await expect(
    page.getByRole('heading', { name: 'Investigate ambiguous evidence' }),
  ).toBeVisible();
  await expect(
    page.getByRole('button', { name: 'Investigate ambiguous evidence' }),
  ).toBeDisabled();
  await expect(page.getByText(/AI is disabled or unavailable/)).toBeVisible();
  await expectAccessible(page);
  await page.goto(`${batchUrl}/settlements/set_3102_p09`);
  await expect(
    page.getByRole('heading', { name: 'Investigate ambiguous evidence' }),
  ).not.toBeVisible();
  await expectAccessible(page);
  await page.goto(`${batchUrl}/overview`);
  await expectAccessible(page);
  ...
  await page.screenshot({
    path: 'test-results/settlements-390x844.png',
    fullPage: true,
  });
  await page.setViewportSize({ width: 1440, height: 900 });
  await page.getByRole('link', { name: 'Settlements' }).click();
  ...
  await expect(
    page.getByRole('heading', { name: 'Razorpay → Bank → Ledger' }),
  ).toBeVisible();
  ...
  await page.keyboard.press('Tab');
  await expect(
    page.getByRole('button', { name: 'Close audit explanation' }),
  ).toBeFocused();
  ...
  await expect(
    page.getByText('Cited source IDs — each copyable').first(),
  ).toBeVisible();
  await expectAccessible(page);
  const exports = [
