...
describe('typed API pagination and exports', () => {
  it('does not send a browser-selected limit and follows every settlement page', async () => {
    const fetchMock = vi.fn((input: RequestInfo | URL) => {
      ...
      const body =
        offset === '0'
          ? {
              batch_id: 'b',
              items: [{ aggregate: { settlement_id: 's1' } }],
              total: 2,
              offset: 0,
              limit: 1,
              next_offset: 1,
          ...
          : {
              batch_id: 'b',
              items: [{ aggregate: { settlement_id: 's2' } }],
              total: 2,
              offset: 1,
              limit: 1,
              next_offset: null,
    ...
    expect(result.items.map((item) => item.aggregate.settlement_id)).toEqual([
      's1',
      's2',
    ]);
    expect(fetchMock.mock.calls.map(([input]) => String(input))).toEqual([
      '/api/v1/batches/b/settlements?offset=0',
      '/api/v1/batches/b/settlements?offset=1',
    ]);
  });
  ...
  it('follows every investigation history page', async () => {
    const fetchMock = vi.fn((input: RequestInfo | URL) => {
      const offset = new URL(String(input), 'http://localhost').searchParams.get(
        'offset',
      );
      const body =
        offset === '0'
    ...
    vi.stubGlobal('fetch', fetchMock);
    const result = await api.listInvestigations('b', 's');
    expect(result.items.map((item) => item.run_id)).toEqual(['run-1', 'run-2']);
