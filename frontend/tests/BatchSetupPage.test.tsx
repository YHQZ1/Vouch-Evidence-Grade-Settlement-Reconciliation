...
describe('batch setup isolation', () => {
  it('requires an explicit new-batch transition and does not retain source slots', async () => {
    ...
    expect(
      screen.getByRole('button', { name: 'Start a new batch' }),
    ).toBeInTheDocument();
    ...
    expect(
      screen.queryByRole('heading', { name: 'Attach immutable source records' }),
  ...
  it('keeps an incomplete or failed batch from looking runnable', async () => {
    vi.spyOn(api, 'createBatch').mockResolvedValue({
      ...
      result_available: false,
      result_batch_id: null,
      failure: { code: 'SOURCE_INVALID', message: 'source rejected', sequence: 2 },
      created_at: '2026-08-31T18:30:00Z',
      updated_at: '2026-08-31T18:30:00Z',
