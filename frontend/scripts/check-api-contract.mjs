...
const code = 'import json; from app.main import app; print(json.dumps(app.openapi(), sort_keys=True, indent=2))'
...
let snapshot
try { snapshot = readFileSync(snapshotPath, 'utf8') } catch { console.error('Missing generated OpenAPI snapshot. Run npm run api:generate.'); process.exit(1) }
if (live !== snapshot) { console.error('Phase 6 OpenAPI drift detected. Run npm run api:generate and review the typed client.'); process.exit(1) }
const schema = JSON.parse(snapshot)
const generatedPath = resolve(root, 'src/types/generated.ts')
let generated
try { generated = readFileSync(generatedPath, 'utf8') } catch { console.error('Missing generated TypeScript contract. Run npm run api:generate.'); process.exit(1) }
const generatedAst = await openapiTS(schema, { alphabetize: true, exportType: 'named' })
const sourceFile = ts.createSourceFile('generated.ts', '', ts.ScriptTarget.Latest, false, ts.ScriptKind.TS)
const printer = ts.createPrinter({ newLine: ts.NewLineKind.LineFeed })
const expectedBody = generatedAst.map((node) => printer.printNode(ts.EmitHint.Unspecified, node, sourceFile)).join('\n\n')
const expected = `/** Generated from src/types/openapi.json. Do not edit manually. */\n${expectedBody}`
if (generated !== expected) { console.error('Generated TypeScript contract is stale. Run npm run api:generate.'); process.exit(1) }
const consumedOperations = [
  ...
  ['/api/v1/batches/{batch_id}/exports/reconciliation-result', 'get', 'exportReconciliationResult'],
...
for (const [path, method, operationId] of consumedOperations) {
  ...
  if (!operation || operation.operationId !== operationId) {
    console.error(`Consumed API operation missing or changed: ${method.toUpperCase()} ${path} (${operationId})`)
    process.exit(1)
  }
}
...
for (const [path, method] of consumedOperations) collectSchemaRefs(schema.paths[path][method])
const apiTypes = readFileSync(resolve(root, 'src/types/api.ts'), 'utf8')
for (const [, name] of apiTypes.matchAll(/components\["schemas"\]\["([^"]+)"\]/g)) visitSchema(name)
for (const name of consumedSchemas) if (!schema.components?.schemas?.[name]) { console.error(`Consumed API schema missing: ${name}`); process.exit(1) }
console.log(`API contract OK: ${consumedOperations.length} operations and ${consumedSchemas.size} consumed schemas checked`)
